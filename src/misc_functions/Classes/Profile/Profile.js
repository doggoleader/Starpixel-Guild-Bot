...
\`Ультраредкие предметы\` - ${userData.box_chances.RNG}x`)
    ...
    async getAboutMember() {
        const userData = await this.getUserData()
        const bdata = await Birthday.findOne({ userid: userData.userid, guildid: userData.guildid })
        let day
        let month
...
\`День рождения\` - ${bday}`)
    ...
    async getAllProfile() {
        ...
        const actexp = await this.getActivityExp()
        ...
        const chances = await this.getChances()
        const about = await this.getAboutMember()

        return [general, progress, actexp, elems, achs, perks, upgrades, gexp, quests, shops, mults, color, chances, about]
    }
}
