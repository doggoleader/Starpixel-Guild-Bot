...
async function Spooky(interaction, client) {
    try {
        ...
        if (roles.cache.has("893932177799135253") || roles.cache.has("567689925143822346")) { //Проверка роли коробки || правления
            ...
            //Список предметов
            let { loot, rank_exp, act_exp } = require(`./Box loot/spooky.json`)
...
╚━═━︽︾︽︾🎃︾︽︾︽━═━╝
◾`)
            if (loot[i_loot].loot_name == `ХЭЛЛОУИНСКАЯ ДУША`) {
                if (userData.seasonal.halloween.hw_soul == false) {
...
╚═════════♡════════╝`
            ...
            console.log(chalk.blackBright(`[${new Date()}]`) + chalk.magentaBright(`[${interaction.user.tag} открыл жуткую коробку]`) + chalk.gray(`: +${act_exp[i_act].act_amount} опыта активности, +${rank_exp[i_rank].rank_amount} опыта рангов и ${loot[i_loot].loot_name}`))
