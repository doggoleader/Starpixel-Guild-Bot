...
╚═════════♡════════╝`
            ...
            userData.cooldowns.daily = Date.now() + (1000 * 60 * 60 * 16) * (1 - (userData.perks.decrease_cooldowns * 0.1)) //ДОБАВИТЬ В ДРУГИЕ(ГДЕ КУЛДАУН)  * 60 * 16
            if (userData.cd_remind.includes('daily')) {
                let ITEM_ID = userData.cd_remind.findIndex(item_id => item_id == 'daily')
