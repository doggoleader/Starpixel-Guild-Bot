...
async function Weekly(interaction, client) {
    try {
        ...
        if (roles.cache.has("504887113649750016")) { //Проверка роли участника гильдии
            ...
            //Список предметов
            let { loot, rank_exp, act_exp } = require(`./Box loot/weekly.json`)
            //рандом предметов
            let chances = []
            let sum_loot = 0;
...
╚═════════♡════════╝`
            ...
            userData.cooldowns.weekly = Date.now() + (1000 * 60 * 60 * 24 * 7) * (1 - (userData.perks.decrease_cooldowns * 0.1)) //ДОБАВИТЬ В ДРУГИЕ(ГДЕ КУЛДАУН)   * 60 * 24 * 7
            if (userData.cd_remind.includes('weekly')) {
                let ITEM_ID = userData.cd_remind.findIndex(item_id => item_id == 'weekly')
                userData.cd_remind.splice(ITEM_ID, 1)
            }

            userData.save();
            client.ActExp(userData.userid)
            console.log(chalk.blackBright(`[${new Date()}]`) + chalk.magentaBright(`[${interaction.user.tag} открыл еженедельную коробку]`) + chalk.gray(`: +${act_exp[i_act].act_amount} опыта активности, +${rank_exp[i_rank].rank_amount} опыта рангов, и ${loot[i_loot].loot_name}`))
