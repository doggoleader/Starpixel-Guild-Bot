...
**Статус**: \`${userData.seasonal.halloween.quest.finished ? `✅ Завершён` : `❌ Не завершён`}\``)
            ...
            const bingo = require(`../../../jsons/HalloweenBingo.json`)
            if (userData.seasonal.halloween.bingo.length <= 0) {
                userData = await createBingoProfile(userData, "halloween", bingo);
            }

            const bigMap = await bingo.bingo.map(async (array, i) => {
                let it = 0;
                const mapAsync = array.map(async task => {
                    let item = userData.seasonal.halloween.bingo[it].find(it => it.id == task.id);
                    while (!item) {
                        it++;
                        item = userData.seasonal.halloween.bingo[it].find(it => it.id == task.id);
                    }
            ...
            let rew_arr = userData.seasonal.halloween.bingo_rewards;
            ...
            let TaskProgress = new ActionRowBuilder()
                .addComponents(
                    new ButtonBuilder()
                        .setCustomId(`hw_bingo_progress`)
                        .setLabel(`Прогресс заданий`)
                        .setStyle(ButtonStyle.Primary)
                )
            const file = new AttachmentBuilder()
                .setFile(`./src/assets/Seasonal/HalloweenBingo.png`)
                .setName(`HalloweenBingo.png`)
            let embed = new EmbedBuilder()
                .setColor(Number(client.information.bot_color))
                .setImage(`attachment://${file.name}`)
                .setDescription(`## ХЭЛЛОУИНСКИЙ БИНГО-МАРАФОН
...
**Если у вас имеются неполученные награды, то нажмите на кнопки ниже, чтобы получить их!**`)
 ...
 \`Диагональ 2\` -> ${rew_arr.includes(`Д2`) ? "✅" : '❌'}`,
            ...
            collector.on('collect', async i => {
                if (i.customId !== 'hw_bingo_progress') {
                    let msg_text = ``;
                    switch (i.customId) {
                        case `r1`: {
                            let index = 0
                            ...
                            for (let item of bingo.bingo[index]) {
                                let usItem = userData.seasonal.halloween.bingo[index].find(it => it.id == item.id);
                            ...
                            userData.seasonal.halloween.bingo_rewards.push(`Р1`);
                        ...
                        case `r2`: {
                            ...
                            userData.seasonal.halloween.bingo_rewards.push(`Р2`);
                        ...
                        case `r3`: {
                            ...
                            userData.seasonal.halloween.bingo_rewards.push(`Р3`);
                        ...
                        case `r4`: {
                            ...
                            userData.seasonal.halloween.bingo_rewards.push(`Р4`);
                        ...
                        case `r5`: {
                            ...
                            userData.seasonal.halloween.bingo_rewards.push(`Р5`);
                        ...
                        case `s1`: {
                            ...
                            for (let item of bingo.bingo) {
                                let usItem = userData.seasonal.halloween.bingo[index2].find(it => it.id == item[index].id)
                                result.push(usItem.finished)
                                index2++
                            }
                            ...
                            userData.seasonal.halloween.bingo_rewards.push(`С1`);
                        ...
                        case `s2`: {
                            ...
                            userData.seasonal.halloween.bingo_rewards.push(`С2`);
                        ...
                        case `s3`: {
                            ...
                            userData.seasonal.halloween.bingo_rewards.push(`С3`);
                        ...
                        case `s4`: {
                            ...
                            userData.seasonal.halloween.bingo_rewards.push(`С4`);
                        ...
                        case `s5`: {
                            ...
                            userData.seasonal.halloween.bingo_rewards.push(`С5`);
                        ...
                        case `d1`: {
                            ...
                            for (let index = 0; index < 5; index++) {
                                let usItem = userData.seasonal.halloween.bingo[index].find(it => it.id == bingo.bingo[index][index].id)
                                result.push(usItem.finished)
                            }
                            ...
                            userData.seasonal.halloween.bingo_rewards.push(`Д1`);
                        ...
                        case `d2`: {
                            ...
                            for (let index1 = 0, index2 = 4; index1 < 5 && index2 >= 0; index1++, index2--) {
                                let usItem = userData.seasonal.halloween.bingo[index1].find(it => it.id == bingo.bingo[index1][index2].id)
                                result.push(usItem.finished)
                            }
                            ...
                            userData.seasonal.halloween.bingo_rewards.push(`Д2`);
                        ...
                        case `all`: {
                            ...
                            let result = []
                            for (let usItems of userData.seasonal.halloween.bingo) {
                            ...
                            userData.seasonal.halloween.bingo_rewards.push(`ВСЕ`);
                    ...
                    userData.save();
                    rew_arr = userData.seasonal.halloween.bingo_rewards;
...
\`Диагональ 2\` -> ${rew_arr.includes(`Д2`) ? "✅" : '❌'}`,
                ...
                } else if (i.customId == `hw_bingo_progress`) {
                    await i.deferReply({ ephemeral: true, fetchReply: true })
                    let a = 0;
                    let qNum = 1;
                    let strings = []
                    for (let itemBig of userData.seasonal.halloween.bingo) {
...
- Осталось выполнить: ${left} ${left <= 0 ? `**ЗАДАНИЕ ВЫПОЛНЕНО** ✅` : ``}\n`)
                    ...
                    .setColor(Number(client.information.bot_color))
                    .setDescription(`## Прогресс заданий хэллоуинского Бинго-марафона
