...
async function execute(interaction, client) {
    try {
        ...
        let number = interaction.fields.getTextInputValue("number")
        try {
            Number(number)
        } catch (err) {
            return interaction.editReply({
                content: `\`${number}\` не является числом!`,
                ephemeral: true
            })
        }
        let option = pollData.options[Number(number) - 1]
        if (!option) return interaction.editReply({
            content: `Данной опции не существует!`,
            ephemeral: true
        })
        pollData.options.splice(Number(number) - 1, 1)
        pollData.save()
        let map
