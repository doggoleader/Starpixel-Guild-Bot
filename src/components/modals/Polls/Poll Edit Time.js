        ...
        try {
            ...
            try {
                ...
                return interaction.editReply({
                    content: `\`${time}\` не является числом!`,
                    ephemeral: true
                })
            }
            if (Number(time) < 1 || Number(time) > 43200) return interaction.editReply({
                content: `Количество минут не должно быть меньше 1 и больше 43200 (30 дней)!`,
                ephemeral: true
            })
            
            pollData.expire_minutes = Number(time);
            pollData.save()
            let map
