...
async function execute(interaction, client) {
    try {
        ...
        let check = await pollData.options.find(o => o.label == label)
        if (check) return interaction.editReply({
            content: `Опция с таким названием уже существует!`,
            ephemeral: true
        })
        let emoji = interaction.fields.getTextInputValue("emoji")
        if (emoji) {
            if (isOneEmoji(emoji) == false) return interaction.editReply({
                content: `Данный символ не является эмоджи или не поддерживается!`,
                ephemeral: true
            })
            await pollData.options.push({
                label: label,
                value: label,
