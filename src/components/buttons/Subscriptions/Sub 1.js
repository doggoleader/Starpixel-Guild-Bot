...
async function execute(interaction, client) {
    try {
        ...
        if (!member.roles.cache.has(`1007290181038133269`)) return interaction.reply({
            embeds: [embed],
            ephemeral: true
        })
...
◾`
        ...
        userData.cooldowns.sub_1 = Date.now() + (1000 * 60 * 60 * 24 * 7) * (1 - (userData.perks.decrease_cooldowns * 0.1))
        if (userData.cd_remind.includes('sub_1')) {
            let ITEM_ID = userData.cd_remind.findIndex(item_id => item_id == 'sub_1')
