...
const { secondPage, divideOnPages, changeProperty, getProperty } = require(`../../../functions`);
...
async function execute(interaction, client) {
    try {
        ...
        if (userData.rank_number < 3) return interaction.reply({
            content: `Вы должны быть **мастером гильдии** или выше, чтобы приобретать товары в данном магазине!`,
            ephemeral: true
        })
        const guildData = await Guild.findOne({ id: interaction.guild.id })
        const shop = require(`../../../jsons/SH.json`)
        let shopForSelMenu = []
                ...
                if (userData.rumbik < price) return i.editReply({
                    ...
                    for (let item of purchased.items) {
                        if (item.type == `role`) {
                            ...
                            await changeProperty(userData, item.db_code, await getProperty(userData, item.db_code) + item.amount)
                            notes.push(`**${a++}**. ${item.amount}x ${item.name} было успешно приобретено! Теперь у вас ${await getProperty(userData, item.db_code)} ${item.name}`)
                        }
                    }
