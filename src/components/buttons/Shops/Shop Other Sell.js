...
async function execute(interaction, client) {
    try {
        ...
        if (userData.rank_number < 9) return interaction.reply({
            content: `Вы должны быть **императором гильдии** или выше, чтобы продавать товары в данном магазине!`,
            ephemeral: true
        })
        const guildData = await Guild.findOne({ id: interaction.guild.id })
        const shop = require(`../../../jsons/OtherShop.json`).filter(item => item.sellable == true)
        let shopForSelMenu = []
        ...
        let items = shop.map((item, i) => {
            pers_price = Math.round((item.price * guildData.global_settings.shop_prices * ((userData.king_costs + userData.shop_costs + userData.act_costs) / 3)) / 2)
...
${list.join(`\n`)}`
        ...
        const list = new EmbedBuilder()
            .setTitle(`Список товаров магазина прочих товаров`)
...
Доступно продаж на сегодня: ${userData.upgrades.max_sells - userData.daily.sells}`)
                ...
                let price = Math.round((sold.price * guildData.global_settings.shop_prices * ((userData.king_costs + userData.shop_costs + userData.act_costs) / 3)) / 2)
