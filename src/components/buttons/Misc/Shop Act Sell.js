...
async function execute(interaction, client) {
    try {
        ...
        if (userData.rank_number < 5) return interaction.reply({
            content: `Вы должны быть **звёздочкой гильдии** или выше, чтобы продавать товары в данном магазине!`,
            ephemeral: true
        })
        const guildData = await Guild.findOne({ id: interaction.guild.id })
        const shop = require(`../../../jsons/AC.json`).filter(item => item.sellable == true)
        let shopForSelMenu = []
        ...
        let items = shop.map((item, i) => {
            pers_price = Math.round((item.price * guildData.global_settings.shop_prices * userData.act_costs) / 2)
            currency = `🏷`
                ...
                for (let item of sold.items) {
                    if (item.type == `role`) {
                        if (i.member.roles.cache.has(item.id)) {
                            if (item.expire <= 0) {
                                ...
                                if (!temp) {
                                    ...
                                    temp.delete()
                                    notes.push(`**${a++}**. Так как этот предмет \`${item.name}\` временный, то его цена была автоматически посчитана за количество оставшихся часов (${leftHours} ч., ~${Math.round(pricePerHour * leftHours)}🏷 получено)`)
                                    await i.member.roles.remove(item.id)
                                }
                            }
                        ...
                        if (!temp) {
                            ...
                            /**
                             * Доделать продажу временных предметов.
                             */
                            let totalHours = Math.round(item.expire * (userData.perks.temp_items + 1) / (60 * 60))
                            let leftHours = Math.round((temp.expire.getTime() - (Date.now())) / (1000 * 60 * 60)) // часов осталось
                            let pricePerHour = ((price / sold.items.length) / totalHours).toFixed(20)
                    ...
                    userData.progress.items.find(it => it.name == 'TICKETS_TOTAL').total_items += price
                    userData.sell.other += 1
                    userData.sell.total_sum += price * 30
