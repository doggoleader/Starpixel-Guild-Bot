...
async function execute(interaction, client) {
    try {
        ...
        let role = `584811380117471252`
        let name = `№10. Вот это коллекция!`
...
Если вы считаете, что это ошибка, напишите об этом в <#${ch_list.ask}>!`)
        ...
        if (!member.roles.cache.has(`531158683883929602`) && !member.roles.cache.has(`531158275400531988`) && !member.roles.cache.has(`553660120379293696`) && !member.roles.cache.has(`553660121444515842`) && !member.roles.cache.has(`931866162508230696`)) return interaction.reply({
            embeds: [no_condition],
            ephemeral: true
        })
