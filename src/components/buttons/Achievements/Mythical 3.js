...
async function execute(interaction, client) {
    try {
        ...
        let role = `694914074550468758`
        let name = `№3. Жертва века`
...
Если вы считаете, что это ошибка, напишите об этом в <#${ch_list.ask}>!`)
        ...
        if (!member.roles.cache.has(`781069821953441832`) || !member.roles.cache.has(`930520087797051452`)) return interaction.reply({
            embeds: [no_condition],
            ephemeral: true
        })
        ...
        await member.roles.remove(`781069821953441832`)
