...
async function execute(interaction, client) {
    try {
        ...
        let role = `585175168251592714`
        let name = `№13. Точно не пуговица`
...
Если вы считаете, что это ошибка, напишите об этом в <#${ch_list.ask}>!`)
        ...
        if (!member.roles.cache.has(`572124468189593622`) && !member.roles.cache.has(`572124606870192143`) && !member.roles.cache.has(`572124610481487890`) && !member.roles.cache.has(`572124614050840576`)) return interaction.reply({
            embeds: [no_condition],
            ephemeral: true
        })
        let reward = `521248091853291540`
        if (member.roles.cache.has(reward)) {
            userData.stacked_items.push(reward)
