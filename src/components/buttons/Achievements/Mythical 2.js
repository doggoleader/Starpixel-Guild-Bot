...
Если вы считаете, что это ошибка, напишите об этом в <#${ch_list.ask}>!`)
        ...
        if (!member.roles.cache.has(`660236704971489310`) || !member.roles.cache.has(`740241985155366973`) || !member.roles.cache.has(`730891493375475786`) || !member.roles.cache.has(`764198086738051092`) || !member.roles.cache.has(`856866046387683338`) || !member.roles.cache.has(`930520087797051452`)) return interaction.reply({
            embeds: [no_condition],
            ephemeral: true
        })
        let reward = `992820488298578041`
