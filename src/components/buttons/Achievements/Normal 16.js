...
module.exports = {
    ...
    async execute(interaction, client) {
        try {
            ...
            let role = `610133972034387983`
            let name = `№16. Не просто пылинка`
...
Если вы считаете, что это ошибка, напишите об этом в <#${ch_list.ask}>!`)
            ...
            if (!member.roles.cache.has(`609085186738618395`) && !member.roles.cache.has(`609086542681604142`) && !member.roles.cache.has(`781069819838464022`) && !member.roles.cache.has(`785252400608182282`) && !member.roles.cache.has(`781069820053160006`)) return interaction.reply({
                embeds: [no_condition],
                ephemeral: true
            })
