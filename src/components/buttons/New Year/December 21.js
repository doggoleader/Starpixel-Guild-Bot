...
async function execute(interaction, client) {
    try {
        ...
        const result = await userData.seasonal.new_year.adventcalendar.find(cal => cal.num == 21)
        if (result) return interaction.reply({
            content: `Вы уже получили эту награду!`,
            ephemeral: true
        })
        if (d !== 21 || m !== 12) return interaction.reply({
            content: `Этот день ещё не наступил. Пожалуйста, нажмите на эту кнопку 21-го декабря!`,
            ephemeral: true
        })
        userData.exp += 700
        ...
        await interaction.reply({
            content: `Вы получили свою награду за 21-е декабря: \`700\`🌀!`,
            ephemeral: true
        })
    } catch (e) {
