...
async function execute(interaction, client) {
    try {
        ...
        const result = await userData.seasonal.new_year.adventcalendar.find(cal => cal.num == 29)
        if (result) return interaction.reply({
            content: `Вы уже получили эту награду!`,
            ephemeral: true
        })
        if (d !== 29 || m !== 12) return interaction.reply({
            content: `Этот день ещё не наступил. Пожалуйста, нажмите на эту кнопку 29-го декабря!`,
            ephemeral: true
        })
        userData.exp += 1000
        ...
        await interaction.reply({
            content: `Вы получили свою награду за 29-е декабря: \`1000\`🌀!`,
            ephemeral: true
        })
    } catch (e) {
