...
async function execute(interaction, client) {
    try {
        ...
        const result = await userData.seasonal.new_year.adventcalendar.find(cal => cal.num == 24)
        if (result) return interaction.reply({
            content: `Вы уже получили эту награду!`,
            ephemeral: true
        })
        if (d !== 24 || m !== 12) return interaction.reply({
            content: `Этот день ещё не наступил. Пожалуйста, нажмите на эту кнопку 24-го декабря!`,
            ephemeral: true
        })
        ...
        await interaction.reply({
            content: `Вы получили свою награду за 24-е декабря: ${role}!`,
            ephemeral: true
        })
    } catch (e) {
