...
async function execute(interaction, client) {
    try {
        ...
        const result = await userData.seasonal.new_year.adventcalendar.find(cal => cal.num == 22)
        if (result) return interaction.reply({
            content: `Вы уже получили эту награду!`,
            ephemeral: true
        })
        if (d !== 22 || m !== 12) return interaction.reply({
            content: `Этот день ещё не наступил. Пожалуйста, нажмите на эту кнопку 22-го декабря!`,
            ephemeral: true
        })
        ...
        await interaction.reply({
            content: `Вы получили свою награду за 22-е декабря: ${role}!`,
            ephemeral: true
        })
    } catch (e) {
