...
module.exports = {
    ...
    async execute(interaction, client) {
        try {
            ...
            const result = await userData.seasonal.new_year.adventcalendar.find(cal => cal.num == 9)
            if (result) return interaction.reply({
                content: `Вы уже получили эту награду!`,
                ephemeral: true
            })
            if (d !== 9 || m !== 12) return interaction.reply({
                content: `Этот день ещё не наступил. Пожалуйста, нажмите на эту кнопку 9-го декабря!`,
                ephemeral: true
            })
            userData.rank += 100
            ...
            await interaction.reply({
                content: `Вы получили свою награду за 9-е декабря: \`100\`💠!`,
                ephemeral: true
            })
        } catch (e) {
