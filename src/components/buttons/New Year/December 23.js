...
async function execute(interaction, client) {
    try {
        ...
        const result = await userData.seasonal.new_year.adventcalendar.find(cal => cal.num == 23)
        if (result) return interaction.reply({
            content: `Вы уже получили эту награду!`,
            ephemeral: true
        })
        if (d !== 23 || m !== 12) return interaction.reply({
            content: `Этот день ещё не наступил. Пожалуйста, нажмите на эту кнопку 23-го декабря!`,
            ephemeral: true
        })
        userData.rank += 200
        ...
        await interaction.reply({
            content: `Вы получили свою награду за 23-е декабря: \`200\`💠!`,
            ephemeral: true
        })
    } catch (e) {
