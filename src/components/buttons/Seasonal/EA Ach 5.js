...
Если вы считаете, что это ошибка, напишите об этом в <#${ch_list.ask}>!`)
        ...
        if (userData.seasonal.easter.achievements.num5 === true) return interaction.reply({
            embeds: [already_done],
            ephemeral: true
        })
...
Если вы считаете, что это ошибка, напишите об этом в <#${ch_list.ask}>!`)
        ...
        if (userData.seasonal.easter.quests_completed < 5) return interaction.reply({
            embeds: [no_condition],
            ephemeral: true
        })
        ...
        userData.seasonal.easter.achievements.num5 = true
...
Он уже получил приз. Хочешь и ты? Тогда тебе в <#${ch_list.ea_achs}>!

Чтобы проверить статистику ваших достижений, используйте меню статистики в канале <#${ch_list.ea_main}>!
