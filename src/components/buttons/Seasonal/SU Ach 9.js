...
Если вы считаете, что это ошибка, напишите об этом в <#${ch_list.ask}>!`)
        ...
        if (userData.seasonal.summer.achievements.num9 === true) return interaction.reply({
            embeds: [already_done],
            ephemeral: true
        })
...
Если вы считаете, что это ошибка, напишите об этом в <#${ch_list.ask}>!`)
        ...
        if (userData.seasonal.summer.unique_quests <= 0) return interaction.reply({
            embeds: [no_condition],
            ephemeral: true
        })
        let reward = `1104095303054934168`
        let name = `Летний жнец`
        ...
        userData.seasonal.summer.achievements.num9 = true
