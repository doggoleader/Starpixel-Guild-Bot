...
Если вы считаете, что это ошибка, напишите об этом в <#${ch_list.ask}>!`)
        ...
        if (userData.seasonal.summer.achievements.num7 === true) return interaction.reply({
            embeds: [already_done],
            ephemeral: true
        })
...
Если вы считаете, что это ошибка, напишите об этом в <#${ch_list.ask}>!`)
        ...
        if (userData.seasonal.summer.points < 100) return interaction.reply({
            embeds: [no_condition],
            ephemeral: true
        })
        let reward = `1104095303054934168`
        let name = `Пляжный патруль`
