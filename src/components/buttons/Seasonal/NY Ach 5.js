...
Если вы считаете, что это ошибка, напишите об этом в <#${ch_list.ask}>!`)
            ...
            const no_condition = new EmbedBuilder()
                ...
                .setDescription(`Вы не соответствуете требованиям для получения данного достижения! Проверить требования вы можете в канале <#${ch_list.ny_achs}>.
    
Если вы считаете, что это ошибка, напишите об этом в <#${ch_list.ask}>!

**Нажмите на кнопку достижения 31 декабря или 1 января!**`)
                .setTimestamp(Date.now())

            const date = new Date();
            let fullDate = `${date.getDate()}.${date.getMonth() + 1}`
            if (fullDate !== `31.12` && fullDate !== `1.1`) return interaction.reply({
                embeds: [no_condition],
                ephemeral: true
            })
            let reward = `595966177969176579`
            let name = `С Новым годом!`
...
Он уже получил достижение! Хочешь и ты? Тогда тебе в <#${ch_list.ny_achs}>

Чтобы проверить статистику ваших достижений, используйте меню статистики в канале <#${ch_list.ny_main}>!`)
