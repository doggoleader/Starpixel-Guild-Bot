...
Если вы считаете, что это ошибка, напишите об этом в <#${ch_list.ask}>!`)
        ...
        if (userData.seasonal.summer.achievements.num10 === true) return interaction.reply({
            embeds: [already_done],
            ephemeral: true
        })
...
Если вы считаете, что это ошибка, напишите об этом в <#${ch_list.ask}>!`)
        ...
        if (userData.seasonal.summer.events.last_event_applied === false) return interaction.reply({
            embeds: [no_condition],
            ephemeral: true
        })
        let reward = `1104095303054934168`
        let name = `Последний закат`
        ...
        userData.seasonal.summer.achievements.num10 = true
